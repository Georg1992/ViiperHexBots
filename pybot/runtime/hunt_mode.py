...
``teleport`` changes area when the hunt is clear; ``walk`` waits without
teleporting.  Legacy INI value ``hybrid`` is normalized to ``walk``.
The controller forwards calls to the active strategy, keeping the OCP
clean: new modes require only a new strategy class, no controller changes.
"""

from __future__ import annotations

from pybot.config.schema import normalize_hunt_mode
...
class HuntModeController:
    ...
    Mode-specific behaviour (teleport, walk) lives in strategy classes.
    The controller preserves the stable public API that workers and tests
    depend on.
    """
...
def create_hunt_mode(
    ...
    * ``"walk"`` → :class:`WalkStrategy`
    """
    mode = normalize_hunt_mode(ctx.config.hunt_mode)
